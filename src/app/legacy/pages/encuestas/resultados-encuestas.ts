...
import { EncuestaService, RespuestaEncuestaDetalleAdminResponse, ResultadoEncuestaAdminResponse } from '../../services/encuesta.service';
...
@Component({
    ...
    template: `
            ...
            <section class="card resultados-hero">
                <div class="resultados-hero__content">
                    <div>
                        ...
                        <p class="resultados-subtitle">Revisa quiÃ©n respondiÃ³ cada encuesta, filtra resultados y exporta Excel general o individual.</p>
                    </div>
                </div>
            </section>
                ...
                <p-table [value]="encuestas" [tableStyle]="{ 'min-width': '82rem' }" responsiveLayout="scroll">
                    ...
                    <ng-template pTemplate="body" let-encuesta>
                        <tr>
                            ...
                            <td>{{ encuesta.descripcion || 'Sin descripciÃ³n' }}</td>
                            <td>
            ...
            <section class="card" *ngIf="encuestaSeleccionada">
                <div class="detalle-header">
                    <div>
                        ...
                        <p class="text-600 mt-2 mb-0">AquÃ­ solo se muestra el participante, correo, fecha y hora de respuesta. Cada respuesta puede exportarse por separado.</p>
                    </div>
                </div>
                ...
                <div class="filters-grid">
                    ...
                    <div class="filter-item">
                        ...
                        <p-select [options]="opcionesAnios" [(ngModel)]="anioSeleccionado" optionLabel="label" optionValue="value" placeholder="Todos" appendTo="body" class="w-full" (onChange)="aplicarFiltrosRespuestas()"></p-select>
                    </div>

                    <div class="filter-item">
                        <label class="filter-label">Mes</label>
                        <p-select [options]="opcionesMeses" [(ngModel)]="mesSeleccionado" optionLabel="label" optionValue="value" placeholder="Todos" appendTo="body" class="w-full" (onChange)="aplicarFiltrosRespuestas()"></p-select>
                    </div>

                    <div class="filter-item">
                        <label class="filter-label">DÃ­a</label>
                        <p-select [options]="opcionesDias" [(ngModel)]="diaSeleccionado" optionLabel="label" optionValue="value" placeholder="Todos" appendTo="body" class="w-full" (onChange)="aplicarFiltrosRespuestas()"></p-select>
                    </div>
        ...
        <p-dialog header="Confirmar eliminaciÃ³n de respuesta" [(visible)]="mostrarDialogoEliminarRespuesta" [modal]="true" [style]="{ width: '32rem', 'max-width': '95vw' }" [closable]="!eliminandoRespuestaId" [draggable]="false" [resizable]="false">
            <div class="pt-2">
                <p class="m-0">
                    Â¿Seguro que deseas eliminar la respuesta de
                    <strong>{{ respuestaSeleccionadaEliminar?.nombreParticipante }}</strong
                    >?
                </p>

                <p class="text-600 mt-3 mb-3">Esta acciÃ³n eliminarÃ¡ el registro completo de esa respuesta.</p>
