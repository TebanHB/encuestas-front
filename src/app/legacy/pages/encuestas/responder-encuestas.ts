...
import { EncuestaDetalleResponse, EncuestaResponse, EncuestaService, GuardarRespuestaRequest } from '../../services/encuesta.service';
                    ...
                    <div class="card">
                        <div class="page-header">
                            <div>
                                ...
                                <p class="page-subtitle">AquÃ­ puedes ver y responder encuestas disponibles.</p>
                            </div>
                        </div>
                            ...
                            <div class="survey-header__content">
                                <div class="survey-header__text">
                                    ...
                                    <p class="survey-description">
                                        {{ encuestaDetalle?.descripcion || 'Completa las respuestas y envÃ­a el formulario.' }}
                                    </p>
                                </div>
                            ...
                            <main class="survey-main">
                                <section class="card section-card participant-card">
                                    <div class="section-header">
                                        ...
                                        <p>Esta encuesta se registrarÃ¡ con los datos del usuario que iniciÃ³ sesiÃ³n.</p>
                                    </div>
                                ...
                                <section class="card question-card" *ngFor="let pregunta of encuestaDetalle.preguntas; let i = index">
                                    ...
                                    <div *ngIf="pregunta.tipo === 'COMPLETAR_ORACION'">
                                        <input pInputText class="w-full" placeholder="Completa la oraciÃ³n" [(ngModel)]="respuestasMap[pregunta.id].textoRespuesta" />
                                    </div>

                                    <div *ngIf="pregunta.tipo === 'NUMERICA'">
                                        <input pInputText class="w-full" placeholder="Ingresa un nÃºmero" [(ngModel)]="respuestasMap[pregunta.id].textoRespuesta" />
                                    </div>
...
export class ResponderEncuestas implements OnInit {
    ...
    enviarRespuestas(): void {
        ...
        if (!this.usuarioLogueado) {
            this.errorMessage = 'No se encontrÃ³ el usuario logueado. Vuelve a iniciar sesiÃ³n.';
            this.cdr.detectChanges();
            return;
        }
        ...
        for (const pregunta of this.encuestaDetalle.preguntas) {
            ...
            if (this.esPreguntaTexto(pregunta.tipo)) {
                ...
                if (pregunta.tipo === 'NUMERICA' && respuesta.textoRespuesta.trim() && isNaN(Number(respuesta.textoRespuesta.trim()))) {
                    this.errorMessage = `La pregunta "${pregunta.titulo}" debe tener un valor numÃ©rico.`;
                    this.cdr.detectChanges();
                    return;
                }
            ...
            if (pregunta.tipo === 'OPCION_UNICA' || pregunta.tipo === 'SI_NO') {
                if (pregunta.obligatoria && !respuesta.opcionUnicaId) {
                    this.errorMessage = `Debes seleccionar una opciÃ³n para la pregunta ${pregunta.titulo}.`;
                    this.cdr.detectChanges();
                    return;
                }
            ...
            if (pregunta.tipo === 'OPCION_MULTIPLE') {
                if (pregunta.obligatoria && respuesta.opcionMultipleIds.length === 0) {
                    this.errorMessage = `Debes seleccionar al menos una opciÃ³n para la pregunta ${pregunta.titulo}.`;
                    this.cdr.detectChanges();
                    return;
                }
