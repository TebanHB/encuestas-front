...
const COLOR_MAP_ES: Record<string, string> = {
    'emerald': 'Esmeralda', 'green': 'Verde', 'lime': 'Lima', 'orange': 'Naranja',
    'amber': 'Ámbar', 'yellow': 'Amarillo', 'teal': 'Verde Azulado', 'cyan': 'Cian',
    'sky': 'Cielo', 'blue': 'Azul', 'indigo': 'Índigo', 'violet': 'Violeta',
    'purple': 'Púrpura', 'fuchsia': 'Fucsia', 'pink': 'Rosa', 'rose': 'Rosado'
};

const COLOR_MAP_REV: Record<string, string> = Object.fromEntries(
    Object.entries(COLOR_MAP_ES).map(([k, v]) => [v, k])
);
COLOR_MAP_REV['noir'] = 'noir';
...
export class AppConfigurator implements OnInit {
    ...
    selectedPrimaryColor = computed(() => {
        const val = this.layoutService.layoutConfig().primary;
        return val && COLOR_MAP_REV[val] ? val : 'Esmeralda';
    });
    ...
    primaryColors = computed<SurfacesType[]>(() => {
        ...
        const colors = Object.keys(COLOR_MAP_ES);
        const palettes: SurfacesType[] = [{ name: 'noir', palette: {} }];

        colors.forEach((color) => {
            palettes.push({
                name: COLOR_MAP_ES[color],
                palette: presetPalette?.[color as KeyOfType<typeof presetPalette>] as any
            });
        });
    ...
    private getColorKey(): string {
        const val = this.layoutService.layoutConfig().primary;
        return COLOR_MAP_REV[val] || 'emerald';
    }

    getPresetExt() {
        const colorKey = this.getColorKey();
        const preset = this.layoutService.layoutConfig().preset;
        const presetPalette = presets[preset as KeyOfType<typeof presets>].primitive;
        const colorPalette = presetPalette?.[colorKey as KeyOfType<typeof presetPalette>] as any;
        const color: SurfacesType = { name: this.selectedPrimaryColor(), palette: colorPalette };

        if (!colorPalette || color.name === 'noir') {
            return {
                semantic: {
    ...
    updateColors(event: Event, type: string, color: SurfacesType) {
        if (type === 'primary') {
            this.layoutService.layoutConfig.update((state) => ({ ...state, primary: color.name || 'Esmeralda' }));
        } else if (type === 'surface') {
            this.layoutService.layoutConfig.update((state) => ({ ...state, surface: color.name || null }));
        }
        setTimeout(() => this.aplicarTemaActual(), 50);
